...
use crate::led_strip_patterns::{
    DEFAULT_PATTERN_INDEX, DEVMINDS_LAMP_LED_NUM, LED_STRIP_PATTERNS, LedStripPattern,
};
...
impl LedStripPatternGenerator {
    ...
    pub fn next_pattern(&mut self) -> &'static LedStripPattern {
        let next_pattern = (self.current_pattern + 1) % LED_STRIP_PATTERNS.len();
        self.activate_pattern(next_pattern);
        self.current_pattern()
    }

    /// Selects the previous pattern in the list and returns it.
    pub fn previous_pattern(&mut self) -> &'static LedStripPattern {
        let previous_pattern = if self.current_pattern == 0 {
            LED_STRIP_PATTERNS.len() - 1
        } else {
    ...
    pub fn default_pattern(&mut self) -> &'static LedStripPattern {
        self.activate_pattern(DEFAULT_PATTERN_INDEX);
        self.current_pattern()
    }

    /// Returns the currently active pattern descriptor.
    pub fn current_pattern(&self) -> &'static LedStripPattern {
        &LED_STRIP_PATTERNS[self.current_pattern]
    }
    ...
    fn activate_pattern(&mut self, index: usize) {
        self.current_pattern = index % LED_STRIP_PATTERNS.len();
...
mod tests {
    use super::*;
    use crate::led_strip_patterns::{DEFAULT_PATTERN_INDEX, LED_STRIP_PATTERNS};
    ...
    fn default_static_pattern_changes_once_until_pattern_changes() {
        ...
        assert_eq!(
            ...
            LED_STRIP_PATTERNS[DEFAULT_PATTERN_INDEX].name
        );
    }
    ...
    fn next_pattern_wraps() {
        ...
        for expected_name in LED_STRIP_PATTERNS
            .iter()
            .skip(1)
    ...
    fn previous_pattern_wraps() {
        ...
        assert_eq!(
            ...
            LED_STRIP_PATTERNS[LED_STRIP_PATTERNS.len() - 1].name
        );
        assert_eq!(
            lamp.next_pattern().name,
            LED_STRIP_PATTERNS[DEFAULT_PATTERN_INDEX].name
